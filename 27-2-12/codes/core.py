...
class FallDetection():
    def __init__(self):
        self.BATCH_SIZE = 13
        self.values = {'t': deque(maxlen=self.BATCH_SIZE), 'x': deque(maxlen=self.BATCH_SIZE),
        'y': deque(maxlen=self.BATCH_SIZE), 'z': deque(maxlen=self.BATCH_SIZE)}
    ...
    async def echo(self, websocket, path):
        async for message in websocket:
            if path == '/accelerometer':
                ...
                self.values = prepare_data(data, self.values)

                has_fallen = predict(self.values, self.BATCH_SIZE)

                if has_fallen:
                    print("The person has fallen, help him/her!")
